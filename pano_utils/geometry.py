...
def cartesian_to_equirectangular_coordinates(v, shape):
  ...
  x = width * tf.math.floormod(azimuth, 2.0 * math.pi) / (2.0 * math.pi)
...
def generate_equirectangular_grid(shape):
  ...
  Raises:
    ValueError: 'shape' is not valid.
  """
  with tf.name_scope('generate_equirectangular_grid'):
    if not isinstance(shape, list) or len(shape) != 2:
      raise ValueError("'shape' is not valid.")
    height, width = shape[0], shape[1]
    pixel_w = 2.0 * math.pi / float(width)
...
def equirectangular_padding(images, num_paddings):
  ...
  """
  with tf.name_scope('equirectangular_padding'):
    if images.ndim != 4:
    ...
    # Symmetric padding with 180° wrap.
    top_padding = tf.reverse(
        tf.roll(images[:, :top, :, :], shift=semicircle, axis=2), axis=[1])
    bottom_padding = tf.roll(
        tf.reverse(images, axis=[1])[:, :down, :, :], shift=semicircle, axis=2)
    padded_images = tf.concat([top_padding, images, bottom_padding], axis=1)
    left_padding = tf.reverse(
        tf.reverse(padded_images, axis=[2])[:, :, :left, :], axis=[2])
    right_padding = padded_images[:, :, :right, :]
    padded_images = tf.concat([left_padding, padded_images, right_padding], axis=2)
