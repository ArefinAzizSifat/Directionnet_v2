...
def world_to_image_projection(p_world, intrinsics, pose_w2c):
    ...
    p_world_h = tf.concat(
        [p_world, tf.ones([height, width, 1], dtype=p_world.dtype)], axis=-1)
    # (1,1,3,4) @ (H,W,4,1) -> (H,W,3)
    p_camera = tf.squeeze(
        tf.matmul(pose_w2c[tf.newaxis, tf.newaxis, :],
                  tf.expand_dims(p_world_h, -1)),
        axis=-1)
    # OpenGL convention flip z
    p_camera = p_camera * \
        tf.constant([1., 1., -1.], dtype=p_camera.dtype, shape=[1, 1, 3])
    # (1,1,3,3) @ (H,W,3,1) -> (H,W,3)
    p_image = tf.squeeze(
        tf.matmul(intrinsics[tf.newaxis, tf.newaxis, :],
                  tf.expand_dims(p_camera, -1)),
        axis=-1)
    z = p_image[:, :, -1:]
...
def image_to_world_projection(depth, intrinsics, pose_c2w):
    ...
    p_image = tf.squeeze(
        tf.matmul(invK, tf.expand_dims(p_pix_h, -1)), axis=-1)  # [H,W,3]

    z = depth * tf.reduce_sum(
        tf.math.l2_normalize(p_image, axis=-1) *
        tf.constant([[[0., 0., 1.]]], dtype=p_image.dtype),
        axis=-1,
        keepdims=True)
    ...
    p_camera = p_camera * \
        tf.constant([1., 1., -1.], dtype=p_camera.dtype, shape=[1, 1, 3])
    p_cam_h = tf.concat(
        [p_camera, tf.ones([height, width, 1], dtype=p_camera.dtype)], axis=-1)

    p_world = tf.squeeze(
        tf.matmul(pose_c2w[tf.newaxis, tf.newaxis, :],
                  tf.expand_dims(p_cam_h, -1)),
        axis=-1)
    return p_world
...
def generate_from_meta(meta_data_path: str,
    ...
    def decode_line(line):
        ...
        splitted = tf.io.decode_csv(line, record_defaults=[
                                    ''] * 10, field_delim=' ')

        img1 = load_single_image(
            pano_data_dir + splitted[0] + '/' + splitted[1] + '.jpeg')
        img2 = load_single_image(
            pano_data_dir + splitted[0] + '/' + splitted[2] + '.jpeg')
...
def generate_random_views(pano1_rgb,
    ...
    swap_yz = tf.constant([[1., 0., 0.], [0., 0., 1.], [
                          0., -1., 0.]], dtype=tf.float32, shape=[1, 3, 3])
    ...
    lookat_directions = tf.concat(
        [lookat_direction1, lookat_direction2], axis=0)
    ...
    sample_rotations = tf.matmul(
        tf.concat([r1, r2], axis=0), lookat_rotations, transpose_a=True)
    ...
    if pano1_depth is not None and pano2_depth is not None:
        ...
        fx = output_width * 0.5 / \
            math.tan(math_utils.degrees_to_radians(output_fov) / 2)
        intrinsics = tf.constant([[fx, 0., output_width * 0.5],
                                  [0., -fx, output_height * 0.5],
                                  [0., 0., 1.]], dtype=tf.float32)

        pose1_c2w = tf.concat(
            [lookat_rotations[0], tf.expand_dims(t1, -1)], axis=1)
        pose2_c2w = tf.concat(
            [lookat_rotations[1], tf.expand_dims(t2, -1)], axis=1)
